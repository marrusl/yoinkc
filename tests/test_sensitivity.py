...
def test_sensitivity_default_strict():
    args = parse_args(["scan", "--from-snapshot", "test.json"])
    assert args.sensitivity == "strict"

def test_sensitivity_moderate():
    args = parse_args(["scan", "--from-snapshot", "test.json", "--sensitivity", "moderate"])
    assert args.sensitivity == "moderate"

def test_sensitivity_strict_explicit():
    args = parse_args(["scan", "--from-snapshot", "test.json", "--sensitivity", "strict"])
    assert args.sensitivity == "strict"

def test_sensitivity_invalid():
    with pytest.raises(SystemExit):
        parse_args(["scan", "--from-snapshot", "test.json", "--sensitivity", "paranoid"])

def test_no_redaction_flag():
    args = parse_args(["scan", "--from-snapshot", "test.json", "--no-redaction"])
...
def test_sensitivity_and_no_redaction_mutual_exclusion():
    with pytest.raises(SystemExit):
        parse_args(["scan", "--from-snapshot", "test.json",
...
def test_sensitivity_passed_to_pipeline(tmp_path):
    args = parse_args(["scan", "--from-snapshot", str(tmp_path / "test.json"),
