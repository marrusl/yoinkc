...
class TestUserInstalledSlimming:

    def test_only_explicit_packages_in_dnf_install(self):
        snapshot = InspectionSnapshot(
            meta={},
            os_release=OsRelease(name="RHEL", version_id="9.6", id="rhel"),
            rpm=RpmSection(
                ...
                user_installed_packages=["httpd", "nginx"],
            ),
        )
        with tempfile.TemporaryDirectory() as tmp:
            render_containerfile(snapshot, _env(), Path(tmp))
        ...
        assert "4 dependencies" in cf or "4 dependency" in cf

    def test_fallback_when_no_user_installed_data(self):
        snapshot = InspectionSnapshot(
            meta={},
            os_release=OsRelease(name="RHEL", version_id="9.6", id="rhel"),
            rpm=RpmSection(
        ...
        assert "httpd" in cf
        assert "apr" in cf
        assert "dependencies pulled in" not in cf
