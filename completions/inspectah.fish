...
# Subcommands
complete -c inspectah -n "not __fish_seen_subcommand_from $subcmds" -a inspect -d "Inspect a host and generate migration artifacts"
...
# Top-level inspect flags for the backwards-compatible `inspectah --flag` form
...
# inspect flags
complete -c inspectah -n "__fish_seen_subcommand_from inspect" -l host-root -r -d "Root path for host inspection"
complete -c inspectah -n "__fish_seen_subcommand_from inspect" -s o -r -d "Write tarball to FILE"
complete -c inspectah -n "__fish_seen_subcommand_from inspect" -l output-dir -r -d "Write files to a directory"
complete -c inspectah -n "__fish_seen_subcommand_from inspect" -l no-subscription -d "Skip bundling RHEL subscription certs"
complete -c inspectah -n "__fish_seen_subcommand_from inspect" -l from-snapshot -r -d "Load snapshot from PATH"
complete -c inspectah -n "__fish_seen_subcommand_from inspect" -l inspect-only -d "Run inspectors only, skip renderers"
complete -c inspectah -n "__fish_seen_subcommand_from inspect" -l target-version -r -d "Target bootc image version"
complete -c inspectah -n "__fish_seen_subcommand_from inspect" -l target-image -r -d "Full target bootc base image reference"
complete -c inspectah -n "__fish_seen_subcommand_from inspect" -l baseline-packages -r -d "Path to baseline package list"
complete -c inspectah -n "__fish_seen_subcommand_from inspect" -l no-baseline -d "Run without base image comparison"
complete -c inspectah -n "__fish_seen_subcommand_from inspect" -l user-strategy -r -a "sysusers blueprint useradd kickstart" -d "Override user creation strategy"
complete -c inspectah -n "__fish_seen_subcommand_from inspect" -l config-diffs -d "Generate line-by-line diffs for modified configs"
complete -c inspectah -n "__fish_seen_subcommand_from inspect" -l deep-binary-scan -d "Full strings scan on unknown binaries"
complete -c inspectah -n "__fish_seen_subcommand_from inspect" -l query-podman -d "Enumerate running containers via podman"
complete -c inspectah -n "__fish_seen_subcommand_from inspect" -l skip-preflight -d "Skip container privilege checks"
complete -c inspectah -n "__fish_seen_subcommand_from inspect" -l validate -d "Run podman build to verify Containerfile"
complete -c inspectah -n "__fish_seen_subcommand_from inspect" -l push-to-github -r -d "Push output to GitHub repository"
complete -c inspectah -n "__fish_seen_subcommand_from inspect" -l github-token -r -d "GitHub personal access token"
complete -c inspectah -n "__fish_seen_subcommand_from inspect" -l public -d "Make new repo public"
complete -c inspectah -n "__fish_seen_subcommand_from inspect" -l yes -d "Skip interactive confirmation prompts"
complete -c inspectah -n "__fish_seen_subcommand_from inspect" -l sensitivity -r -a "strict moderate" -d "Heuristic detection sensitivity"
complete -c inspectah -n "__fish_seen_subcommand_from inspect" -l no-redaction -d "Disable all redaction"
complete -c inspectah -n "__fish_seen_subcommand_from inspect" -l skip-unavailable -d "Skip the package availability preflight check"
