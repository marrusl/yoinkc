...
Filesystem-based service state detection: scans `.wants/` directories in `/host/etc/systemd/system/` for enabled services, symlinks to `/dev/null` for masked services, and vendor units in `/host/usr/lib/systemd/system/` for disabled/static classification. This avoids `systemctl` version mismatches between the tool container and host.
...
A maintainable exclusion list filters out known system-generated files that are not operator-placed configs:
- **Exact paths**: `/etc/.updated`, `/etc/passwd-`, `/etc/shadow-`, `/etc/subuid-`, `/etc/subgid-`, `/etc/resolv.conf`, `/etc/systemd/system/default.target`, `/etc/systemd/system/dbus.service`, `/etc/systemd/user/dbus.service`
- **Glob patterns**: `/etc/alternatives/*`, `/etc/pki/ca-trust/extracted/*`, `/etc/systemd/system/*.wants/*`, `/etc/systemd/user/*.wants/*`, `/etc/systemd/sleep.conf.d/*`, `/etc/lvm/archive/*`, `/etc/lvm/backup/*`, `/etc/lvm/devices/*`, `/etc/firewalld/zones/*.xml.old`, `/etc/pm/sleep.d/*`

The exclusion list is defined as two data structures at the top of the config inspector source, making it easy to extend as new false positives are discovered.
...
Scans `/opt` and `/usr/local` (not `/home` or `/srv` — user home directories are not scanned to avoid noise). For FHS directories under `/usr/local` (`bin`, `sbin`, `libexec`, `lib`, `lib64`), files are enumerated and classified individually rather than treated as opaque directories. `/usr/local/lib` recurses one level into subdirectories.
...
For the Containerfile, generates `.append` fragment files (e.g., `config/etc/passwd.append`) containing only non-system entries. These are appended to the base image's files via `cat >> /etc/passwd` etc., preserving exact UIDs, encrypted passwords, and account flags. Home directories are created with correct ownership. Falls back to `useradd`/`groupadd` commands for snapshots without raw entries.
...
# Detected: 2 additional repos beyond base
COPY config/etc/yum.repos.d/ /etc/yum.repos.d/

# === Package Installation ===
# Detected: 47 packages added beyond base image
...
# === Service Enablement ===
# Detected: 5 non-default services enabled, 1 default service disabled
...
# FIXME: Go binary at /usr/local/bin/mytool (statically linked)
...
# === Users and Groups ===
COPY config/etc/group.append /tmp/group.append
COPY config/etc/passwd.append /tmp/passwd.append
COPY config/etc/shadow.append /tmp/shadow.append
COPY config/etc/gshadow.append /tmp/gshadow.append
RUN cat /tmp/group.append >> /etc/group && \
    cat /tmp/passwd.append >> /etc/passwd && \
    cat /tmp/shadow.append >> /etc/shadow && \
    cat /tmp/gshadow.append >> /etc/gshadow && \
    rm -f /tmp/*.append
...
# Static NM profiles baked into image
COPY config/etc/NetworkManager/system-connections/ /etc/NetworkManager/system-connections/
# DHCP connections deferred to kickstart — see kickstart-suggestion.ks
# FIXME: configure DHCP interfaces via kickstart
...
**Additional distribution support.** RHEL 10 support requires verifying that suitable bootc base images are available and that the RPM/systemd inspection paths haven't changed. Fedora support would be a natural extension given that Fedora has bootc images.
