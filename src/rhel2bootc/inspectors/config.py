...
_UNOWNED_EXCLUDE_EXACT: Set[str] = {
    ...
    "/etc/subuid-",
    "/etc/subgid-",
    # machine-id (generated at first boot)
    "/etc/machine-id",
    # ld.so cache (regenerated by ldconfig)
    "/etc/ld.so.cache",
    # RHEL subscription manager cache
    "/etc/pki/product-default/69.pem",
    # resolv.conf when NM-managed (network inspector handles provenance)
    "/etc/resolv.conf",
    # hostname (set at deploy time, not baked)
    "/etc/hostname",
    # localtime (symlink to zoneinfo, set at deploy time)
    "/etc/localtime",
    # adjtime (hardware clock state)
    "/etc/adjtime",
    # dnf/yum cache state
    "/etc/dnf/dnf.conf",
    "/etc/yum.conf",
    # standard systemd symlinks (created by systemd itself)
...
_UNOWNED_EXCLUDE_GLOBS: List[str] = [
    ...
    # X11 fontpath cache
    "/etc/X11/fontpath.d/*",
    # selinux compiled policy (binary, managed by semodule)
    "/etc/selinux/*/policy/policy.*",
    "/etc/selinux/*/contexts/files/file_contexts.bin",
    # systemd journal machine-id catalog
    "/etc/udev/hwdb.bin",
    # pki anchors regenerated by update-ca-trust
    ...
    # systemctl enable/disable symlinks — already covered by service inspector
    "/etc/systemd/system/*.wants/*",
    "/etc/systemd/system/*.requires/*",
    "/etc/systemd/user/*.wants/*",
    "/etc/systemd/user/*.requires/*",
    # systemd sleep vendor drop-ins
    "/etc/systemd/sleep.conf.d/*",
    # LVM metadata (generated by lvm tools, not operator config)
    "/etc/lvm/archive/*",
    "/etc/lvm/backup/*",
    "/etc/lvm/devices/*",
