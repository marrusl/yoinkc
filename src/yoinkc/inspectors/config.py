...
_UNOWNED_EXCLUDE_EXACT: Set[str] = {
    ...
    # ── ld.so cache (regenerated by ldconfig post-install) ──────────────────
    # Note: /etc/ld.so.conf.d/*.conf files are RPM-owned and never appear
    # in the unowned list, so no glob is needed for that directory.
    "/etc/ld.so.cache",
    ...
    "/etc/sysconfig/network-scripts/readme-ifcfg-rh.txt",  # doc file
