...
_UNOWNED_EXCLUDE_GLOBS: List[str] = [
    ...
    "/etc/crypto-policies/back-ends/*",   # generated by update-crypto-policies
...
def _detect_crypto_policy(host_root: Path, warnings: Optional[list]) -> None:
    """Read the system crypto policy and warn if non-default.

    The crypto policy config file and any custom .pol files are picked up by
    the normal RPM-owned-modified / unowned file scanning.  This method only
    adds a user-facing warning when the active policy isn't DEFAULT.
    """
    cfg = host_root / "etc/crypto-policies/config"
    try:
        if not cfg.exists():
            return
        policy = cfg.read_text().strip().split("\n")[0].strip()
    except (PermissionError, OSError):
        return
    if not policy:
        return
    # Strip inline comments (e.g. "LEGACY  # set by admin")
    policy = policy.split("#")[0].strip()
    if not policy:
        return
    if not _CRYPTO_POLICY_NAME_RE.fullmatch(policy):
        if warnings is not None:
            warnings.append(make_warning(
                "config",
                f"System crypto policy value {policy!r} contains unexpected characters — Containerfile update-crypto-policies command will be skipped",
            ))
        return
    if policy != "DEFAULT" and warnings is not None:
        warnings.append(make_warning(
            "config",
            f"System crypto policy is set to {policy} — base image may use DEFAULT",
        ))
...
def run(
    ...
    _detect_crypto_policy(host_root, warnings)
